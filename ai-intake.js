...
async function tryBackendChat(userMsg, localReply) {
  ...
  try {
    const res = await fetch(`${BACKEND}/v1/legion/military-ai/query`, {
      ...
      body: JSON.stringify({
        query: userMsg,
        context: 'You are the AI assistant for American Legion Post 579, Mrs. Carden Post, located at 3002 Gunsmoke Street, San Antonio TX 78227, phone (210) 674-8069. Answer questions about veteran assistance, membership, hall rental, events, donations, and Post 579 programs. Be concise, friendly, and patriotic.',
      }),
    });
    if (!res.ok) return;
